...
def generate_ai_insights(df, manager_name):
    """AI-powered performance insights using pattern detection"""
    import anthropic
    
    mgr_data = df[df["Manager"] == manager_name]
    
    # Prepare context
    monthly_trend = mgr_data.groupby("Disb Month")["Disbursed AMT"].sum()
    best_month = monthly_trend.idxmax()
    worst_month = monthly_trend.idxmin()
    avg_ticket = mgr_data["Disbursed AMT"].mean()
    
    # Top campaigns
    top_campaigns = mgr_data.groupby("Campaign")["Disbursed AMT"].sum().nlargest(3)
    
    context = f"""
    Manager: {manager_name}
    Best Month: {best_month} (₹{monthly_trend[best_month]/100000:.1f}L)
    Worst Month: {worst_month} (₹{monthly_trend[worst_month]/100000:.1f}L)
    Average Ticket Size: ₹{avg_ticket/100000:.2f}L
    Top 3 Campaigns: {', '.join(top_campaigns.index.tolist())}
    ...
    message = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": f"You are a business intelligence analyst. {context}"
        }]
    )
...
def display_ai_insights_panel(insights):
    """Display AI insights in beautiful cards"""
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);
        ...
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
            ...
            <div>
                <div style="font-size:18px;font-weight:700">AI-Powered Insights</div>
                <div style="font-size:12px;opacity:0.9">Generated by Claude AI</div>
            </div>
        </div>
        <div style="background:rgba(255,255,255,0.15);border-radius:12px;padding:16px;
            backdrop-filter:blur(10px);font-size:13px;line-height:1.8">
