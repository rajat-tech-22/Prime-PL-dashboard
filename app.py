...
if dashboard_type == "All Managers":
    ...
    filtered_df = filtered_for_campaigns.copy()
...
# -----------------------------
    ...
    with col1:
        colored_metric(selected_manager1, f"Disb: {format_inr(d1)} | Count: {txn1}", "#636EFA")
    with col2:
        colored_metric(selected_manager2, f"Disb: {format_inr(d2)} | Count: {txn2}", "#00CC96")
...
# -----------------------------
    ...
    campaign_months = ["All"] + sorted(df["Disb Month"].dropna().unique())

    selected_campaign = st.sidebar.selectbox("Select Campaign", campaigns)
    selected_campaign_month = st.sidebar.selectbox("Select Month", campaign_months)

    filtered_campaign = df.copy()
    if selected_campaign != "All":
        filtered_campaign = filtered_campaign[filtered_campaign["Campaign"] == selected_campaign]
    if selected_campaign_month != "All":
        filtered_campaign = filtered_campaign[filtered_campaign["Disb Month"] == selected_campaign_month]

    st.header("📊 Campaign Performance Dashboard")
    if filtered_campaign.empty:
        st.warning("No data available for selected filters.")
    else:
        campaign_summary = filtered_campaign.groupby("Manager").agg(
            Total_Disbursed=("Disbursed AMT","sum"),
            Disb_Count=("Disbursed AMT","count")
        ).reset_index()
        campaign_summary["Total_Disbursed"] = campaign_summary["Total_Disbursed"].apply(format_inr)
        ...
        fig_campaign = go.Figure(go.Bar(
            x=campaign_summary["Manager"],
            y=[float(str(v).replace("₹","").replace(",","")) for v in campaign_summary["Total_Disbursed"]],
            text=campaign_summary["Total_Disbursed"],
            textposition="auto",
            marker_color=base_colors[:len(campaign_summary)]
        ))
        fig_campaign.update_layout(
            yaxis_title="Disbursed Amount",
            template="plotly_white",
            height=400,
            title=f"Campaign Performance - {selected_campaign} ({selected_campaign_month})"
        )
