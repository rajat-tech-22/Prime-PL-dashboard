...
/* ── Expander header text black bold ── */
...
# Default to current month if present, else latest available
_now_ist = datetime.now(timezone(timedelta(hours=5, minutes=30)))
_current_month_str = _now_ist.strftime("%b-%Y")
if _current_month_str in months:
    latest_month_index = list(months).index(_current_month_str)
else:
...
# ══════════════════════════════════════════
    ...
    # ── Compute current month index for Team vs Month ──
    _tvm_now = datetime.now(timezone(timedelta(hours=5, minutes=30)))
    _tvm_curr = _tvm_now.strftime("%b-%Y")
    _tvm_curr_idx = list(months).index(_tvm_curr) if _tvm_curr in months else latest_month_index
    _tvm_prev_idx = max(0, _tvm_curr_idx - 1)

    # ── DISB DATE range from data ──
    _has_date_col = "DISB DATE" in df.columns and df["DISB DATE"].notna().any()
    if _has_date_col:
        _min_date = df["DISB DATE"].min().date()
        _max_date = df["DISB DATE"].max().date()
    else:
        _min_date = datetime(2024, 1, 1).date()
        _max_date = _tvm_now.date()
    ...
    with st.sidebar.expander("📅 Date Range Filter", expanded=True):
        st.markdown(
            '<p style="color:#000000;font-weight:700;font-size:13px;margin-bottom:6px">'
            'Filter by Disbursement Date</p>',
            unsafe_allow_html=True
        )
    ...
    if use_date_filter and _has_date_col:
        disb_df = disb_df[
            (disb_df["DISB DATE"].dt.date >= date_from) &
            (disb_df["DISB DATE"].dt.date <= date_to)
        ]
        st.info(
            f"📅 Date filter active: **{date_from.strftime('%d %b %Y')}** → "
            f"**{date_to.strftime('%d %b %Y')}**  |  "
            f"{len(disb_df)} records"
        )
