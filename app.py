...
# Main data from first Google Sheet
...
def colored_metric(label, value, color="#000"):
    st.markdown(f"""
    <div style="background-color:{color}; padding:15px; border-radius:10px; text-align:center; color:white; margin-bottom:10px;">
...
if dashboard_type == "All Managers":
    ...
    if "Bank" in filtered_df.columns:
        ...
        if not bank_summary.empty:
            ...
            fig_bank.update_layout(title="Bank-wise Disbursed Amount", yaxis_title="Amount (L)", template="plotly_white")
...
# -----------------------------
    ...
    campaigns_available = sorted(f["Campaign"].dropna().unique())
    selected_campaigns = st.sidebar.multiselect("Campaigns", campaigns_available, default=campaigns_available)
...
# -----------------------------
    ...
    months = sorted(df["Disb Month"].dropna().unique())
    selected_month1 = st.sidebar.selectbox("Month 1", months, index=len(months)-1)
    selected_month2 = st.sidebar.selectbox("Month 2", months, index=len(months)-1)
...
# -----------------------------
    ...
    new_sheet_url = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRfDuP8FJ8yxoZxPPiKYrxWyP1Brlq7hmyxrkp81_oURiPrCJHqup7Ru8wFoE-pWMEZjZIuH_VBM5_i/pub?gid=2057712936&single=true&output=csv"
