...
dashboard_type = st.sidebar.radio("Select View", ["All Managers", "Single Manager", "Campaign Performance", "Comparison"])
st.sidebar.divider()

months = sorted(df["Disb Month"].dropna().unique())
managers = sorted(df["Manager"].dropna().unique())
verticals = ["All"] + sorted(df["Vertical"].dropna().unique())
latest_month_idx = len(months)-1 if months else 0

# -----------------------------
# 1. All Managers Dashboard
# -----------------------------
if dashboard_type == "All Managers":
    selected_month = st.sidebar.selectbox("Select Month", months, index=latest_month_idx)
    ...
    st.header(f"🌎 Enterprise Overview - {selected_month}")
    ...
    with c4: colored_metric("Total Deals", tc, "#FFA15A")

    st.subheader("Manager performance Breakdown")
    agg_df = f.groupby(["Vertical","Manager"]).agg(Disbursed=("Disbursed AMT","sum"), Deals=("Manager","count")).reset_index()
    agg_df.sort_values("Disbursed", ascending=False, inplace=True)
...
# -----------------------------
    ...
    sel_month = st.sidebar.selectbox("Select Month", months, index=latest_month_idx)
    ...
    if f.empty:
        ...
        with col_l: st.plotly_chart(plot_bar(f, "Bank", tb, sel_mgr), use_container_width=True)
        with col_r: st.plotly_chart(plot_bar(f, "Caller", tclr, sel_mgr), use_container_width=True)
...
# -----------------------------
    ...
    sel_mgr = st.sidebar.selectbox("Filter by Manager", ["All"] + managers)
    ...
    if f_camp.empty:
        st.info("Please select campaigns from the sidebar to see performance.")
    else:
        td, tr, ap, tc, _, tb, tcmp, _ = calc_metrics(f_camp)
        c1, c2, c3 = st.columns(3)
        with c1: colored_metric("Selected Disbursed", format_inr(td), "#636EFA")
        with c2: colored_metric("Selected Revenue", format_inr(tr), "#00CC96")
        with c3: colored_metric("Total Leads", tc, "#AB63FA")
        
        st.plotly_chart(plot_bar(f_camp, "Campaign", tcmp, "Campaign Comparison"), use_container_width=True)
...
# -----------------------------
    ...
    st.header("⚖️ Manager vs Manager Benchmark")
    ...
    month_cmp = st.sidebar.selectbox("Select Month", months, index=latest_month_idx)
    
    f1 = df[(df["Manager"]==m1) & (df["Disb Month"]==month_cmp)]
    f2 = df[(df["Manager"]==m2) & (df["Disb Month"]==month_cmp)]
    
    col_a, col_b = st.columns(2)
    for col, manager, data in zip([col_a, col_b], [m1, m2], [f1, f2]):
        with col:
            st.subheader(manager)
            if data.empty: st.write("No data for this month.")
            else:
                td, _, ap, _, _, _, _, _ = calc_metrics(data)
                colored_metric("Disbursed", format_inr(td), "#636EFA")
                colored_metric("Payout %", f"{ap:.2f}%", "#EF553B")
                st.plotly_chart(plot_bar(data, "Bank", "N/A", manager), use_container_width=True)
