...
df = load_data(
    "https://docs.google.com/spreadsheets/d/1I1ql5NwFafbWXYkVOvv0yvMM9GKnJ5954R4zif2owGI/export?format=csv",
    rename_map={"Campaign Name":"Campaign", "Disbursed Amount":"Disbursed AMT"}
)

# Campaign performance sheet
df_campaign = load_data(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vRfDuP8FJ8yxoZxPPiKYrxWyP1Brlq7hmyxrkp81_oURiPrCJHqup7Ru8wFoE-pWMEZjZIuH_VBM5_i/pub?gid=2057712936&single=true&output=csv",
    rename_map={"Campaign Name":"Campaign", "Disbursed Amount":"Disbursed AMT"}
)
...
def calc_metrics(f):
    ...
    total_rev = f.get("Total_Revenue", f["Disbursed AMT"]).sum()
...
def plot_bar(f, col, top_value, title):
    if col not in f.columns or f.empty:
        return go.Figure()
    summary = f.groupby(col)["Disbursed AMT"].sum()
    colors = ["#FFD700" if i==top_value else "#636EFA" for i in summary.index]
    ...
    fig.update_layout(title=title, template="plotly_white", height=400, yaxis_title="Amount (L)")
    return fig
...
with tab1:
    ...
    campaigns = sorted(filtered["Campaign"].dropna().unique())
    ...
    agg_df = filtered.groupby(["Vertical","Manager"]).agg(
        ...
        Total_Revenue=("Total_Revenue","sum") if "Total_Revenue" in filtered.columns else ("Disbursed AMT","sum"),
...
with tab2:
    ...
    if not filtered.empty:
        ...
        st.plotly_chart(plot_bar(filtered,"Bank",top_bank,"Bank Summary"), use_container_width=True)
        st.plotly_chart(plot_bar(filtered,"Caller",top_caller,"Caller Summary"), use_container_width=True)
...
with tab3:
    ...
    d1,r1,p1,txn1,avg1,_,_,_ = calc_metrics(f1)
    d2,r2,p2,txn2,avg2,_,_,_ = calc_metrics(f2)
    col1,col2 = st.columns(2)
