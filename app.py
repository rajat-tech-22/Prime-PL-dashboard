...
# PAGE CONFIG
st.set_page_config(page_title="Prime PL Dashboard", page_icon="💼", layout="wide")
...
if not st.session_state.login:
    ...
    with col2:
        ...
        st.subheader("Welcome to MyMoneyMantra")
        st.caption("Sign in to your account to access the Prime PL Dashboard.")
        ...
        if st.button("Sign In →", use_container_width=True):
            if u == USERNAME and p == PASSWORD:
                ...
                st.error(f"Invalid credentials. Attempt {st.session_state.attempts}/{MAX_ATTEMPTS}")
        
        st.markdown('</div>', unsafe_allow_html=True)
    st.stop()

# --- DASHBOARD CONTENT AFTER LOGIN ---
st.title("📊 Prime PL Dashboard")
