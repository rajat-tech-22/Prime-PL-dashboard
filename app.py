...
# ----------------------------- #
    ...
    fig_comp.add_trace(go.Bar(name=label1, x=comp_df["Metric"], y=comp_df[label1], marker_color="#636EFA"))
    fig_comp.add_trace(go.Bar(name=label2, x=comp_df["Metric"], y=comp_df[label2], marker_color="#EF553B"))

    fig_comp.update_layout(barmode='group', title="Comparison Overview", template="plotly_white")
    ...
    bank1 = f1.groupby("Bank")["Disbursed Amount"].sum().reset_index()
    bank2 = f2.groupby("Bank")["Disbursed Amount"].sum().reset_index()

    bank_merge = pd.merge(bank1, bank2, on="Bank", how="outer",
                          suffixes=(f"_{label1}", f"_{label2}")).fillna(0)

    bank_merge = bank_merge.sort_values(by=f"Disbursed Amount_{label1}", ascending=False).head(10)

    fig_bank = go.Figure()
    fig_bank.add_trace(go.Bar(x=bank_merge["Bank"], y=bank_merge[f"Disbursed Amount_{label1}"]/100000, name=label1))
    fig_bank.add_trace(go.Bar(x=bank_merge["Bank"], y=bank_merge[f"Disbursed Amount_{label2}"]/100000, name=label2))

    fig_bank.update_layout(barmode='group', template="plotly_white")
    st.plotly_chart(fig_bank, use_container_width=True)
    ...
    camp1 = f1.groupby("Campaign")["Disbursed Amount"].sum().reset_index()
    camp2 = f2.groupby("Campaign")["Disbursed Amount"].sum().reset_index()

    camp_merge = pd.merge(camp1, camp2, on="Campaign", how="outer",
                          suffixes=(f"_{label1}", f"_{label2}")).fillna(0)

    camp_merge = camp_merge.sort_values(by=f"Disbursed Amount_{label1}", ascending=False).head(10)

    fig_camp = go.Figure()
    fig_camp.add_trace(go.Bar(x=camp_merge["Campaign"], y=camp_merge[f"Disbursed Amount_{label1}"]/100000, name=label1))
    fig_camp.add_trace(go.Bar(x=camp_merge["Campaign"], y=camp_merge[f"Disbursed Amount_{label2}"]/100000, name=label2))

    fig_camp.update_layout(barmode='group', template="plotly_white")
    ...
    caller1 = f1.groupby("Caller")["Disbursed Amount"].sum().reset_index()
    caller2 = f2.groupby("Caller")["Disbursed Amount"].sum().reset_index()

    caller_merge = pd.merge(caller1, caller2, on="Caller", how="outer",
                            suffixes=(f"_{label1}", f"_{label2}")).fillna(0)

    caller_merge = caller_merge.sort_values(by=f"Disbursed Amount_{label1}", ascending=False).head(10)

    fig_caller = go.Figure()
    fig_caller.add_trace(go.Bar(x=caller_merge["Caller"], y=caller_merge[f"Disbursed Amount_{label1}"]/100000, name=label1))
    fig_caller.add_trace(go.Bar(x=caller_merge["Caller"], y=caller_merge[f"Disbursed Amount_{label2}"]/100000, name=label2))

    fig_caller.update_layout(barmode='group', template="plotly_white")
