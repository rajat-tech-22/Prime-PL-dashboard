...
# -----------------------------
campaign_df = pd.read_csv("YOUR_CAMPAIGN_CSV_URL")  # replace with your CSV URL

# -----------------------------
# Dashboard Type
# -----------------------------
dashboard_type = st.sidebar.selectbox("Select Dashboard", ["📊 Campaign Funnel Analysis"])
...
if dashboard_type == "📊 Campaign Funnel Analysis":
    ...
    c1.metric("IVR Data", f"{total_ivr:,}", delta_color="normal")
    c2.metric("Press 1", f"{press1:,}", delta_color="inverse")
    c3.metric("Leads", f"{leads:,}", delta_color="normal")
    c4.metric("RCS Sent", f"{sent:,}", delta_color="normal")
    c5.metric("RCS Read", f"{read:,}", delta_color="normal")
    c6.metric("Clicks", f"{clicks:,}", delta_color="normal")
    c7.metric("Cost (₹)", f"{cost:,.2f}", delta_color="inverse")
